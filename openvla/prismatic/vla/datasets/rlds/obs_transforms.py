...
def crop_center(image):
    # TODO the h must be smaller than w
    h, w = tf.shape(image)[-3], tf.shape(image)[-2]
    
    c = tf.minimum(w, h)
    cropped_image = tf.image.crop_to_bounding_box(image, tf.maximum(h - c, 0), tf.maximum(w - c, 0)// 2, c, c)
    ...
    for name in image_names:
        ...
        if image.dtype == tf.string:
            if tf.strings.length(image) == 0:
                ...
                # print(image.shape[-3])
                # if image.shape[-3] == 180:
                #     # droid
                #     # crop
                #     image = tf.image.crop_to_bounding_box(image, 0, 70, 180, 180)
        ...
        if name in resize_size:
            if center_crop:
                ...
                image = tf.image.resize_with_pad(image, target_height=resize_size[name][0], target_width=resize_size[name][1], antialias=True)
