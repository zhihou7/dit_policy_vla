...
class VLAMetrics:
    def __init__(
        ...
        self.state = {
            ...
            "action_accuracy_a": deque(maxlen=window_size),
            "step_time": deque(maxlen=window_size),
            "lr": [],
        }
    ...
    def push(self) -> str:
        ...
        loss_raw = torch.stack(list(self.state["loss_raw"])).mean().item() if len(self.state['loss_raw']) > 0 else 0
        diff_loss = torch.stack(list(self.state["diff_loss"])).mean().item() if len(self.state['diff_loss']) > 0  else 0
        loss = torch.stack(list(self.state["loss"])).mean().item() if len(self.state['loss']) > 0   else 0
        l1_loss = torch.stack(list(self.state["l1_loss"])).mean().item() if len(self.state['l1_loss']) > 0  else 0
        action_accuracy = torch.stack(list(self.state["action_accuracy"])).mean().item() if len(self.state['action_accuracy']) > 0  else 0
        action_accuracy_a = torch.stack(list(self.state["action_accuracy_a"])).mean().item() if  len(self.state['action_accuracy_a']) > 0 else 0
        step_time, lr = np.mean(list(self.state["step_time"])), self.state["lr"][-1]
        status = self.get_status(loss)
        ...
        for ds, tracker in self.dataset_trackers.items():
            dataset_metrics.update(
                {
                    f"{ds}/L1 Loss": torch.stack(list(tracker.state["l1_loss"])).mean().item()  if len(tracker.state['l1_loss']) > 0   else 0,
                    f"{ds}/Action Token Accuracy": torch.stack(list(tracker.state["action_accuracy"])).mean().item()  if len(tracker.state['action_accuracy']) > 0   else 0,
                }
            )
        ...
        self.log(
            ...
            metrics={
                ...
                f"{prefix}/Loss": loss,
                f'{prefix}/diff_loss': diff_loss,
                f"{prefix}/L1 Loss": l1_loss,
                f"{prefix}/Action Token Accuracy": action_accuracy,
                f"{prefix}/Action Token Accuracy masked": action_accuracy_a,
                f"{prefix}/Loss (Raw)": loss_raw,
                f"{prefix}/Learning Rate": lr,
                f"{prefix}/Step Time": step_time,
                **dataset_metrics,
